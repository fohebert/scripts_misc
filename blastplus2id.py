...
__authors__ = "Eric Normandeau and Nicolas Maillet, adaptation par Dr. Pants"
__program_name__ = "Blast2id_nr"
__version_info__ = ('0', '0', '2')
__version__ = '.'.join(__version_info__)
__revision_date__ = "2012-03-27"
...
def help():
    ...
    Blast2id(1)\n\n\033[1mNAME\033[0m\n\t%s - Format a result file of Blast+ f\
...
\033[1mAUTHOR\033[0m\n\tWritten by Eric Normandeau & Nicolas Maillet adapted by\
 Dr. Pants\n\
    ...
    for line in input_file:
        ...
        if line.startswith("Query="):
            if new_contig == True:
                output_file.write("\n")
            blast_count = 0
            output_file.write(line.split("Query= ")[1]) # Mon ajout
            #Ce que Éric avait écrit --> current_blast = line.split("Query= ")[1]
            new_contig = True
            complete = False
            ...
            evalue_found = False
            started = True
            output_file.write("\t" + re.split(" ", line)[1])
            ##Ce que Éric avait écrit --> current_blast += "\t" + re.split(" ",line)[1]
        elif line.startswith(">") and started == True:
            "BANG!"
        elif line.startswith("Length=") and started == True:
            started = False
            #Ce que Éric avait écrit --> res += current_blast
            #Ce que Éric avait écrit --> current_blast = ""
        elif line.startswith("***** No hits found *****") and complete == False:
            output_file.write("\t" + "No hits found")
            #Ce que Éric avait écrit --> res += current_blast + "\t" + "No hits found"
            complete = True
            blast_count = -99
            started = False
        elif "Expect" in line and complete == False and evalue_found == False:
            output_file.write("\t" + line.split(" ")[-1].strip())
            #Ce que Éric avait écrit --> res += "\t" + line.split(" ")[-1].strip()
            evalue_found = True
        #Ce que Éric avait écrit --> elif started == True:
        #Ce que Éric avait écrit -->     current_blast += " " + line.strip()
        if blast_count > max_blasts:
            complete = True
            started = False
    #Ce que Éric avait écrit --> res += "\n"
    #Ce que Éric avait écrit --> output_file.write(res)
